...
class CalFileParser
{
  ...
  public function read_file($file = '')
  {
    ...
    if (1 === preg_match('/^(http|https):/', $file)) {
  ...
  public function parse($file = '', $output = '')
  {
    ...
    if (1 === preg_match('/X-WR-TIMEZONE:(.+)/i', $file_contents, $timezone)) {
      $this->_file_timezone = trim($timezone[1]);
      if (null == $this->_user_timezone) {
  ...
  private function convert_event_string_to_array($event_str = '')
  {
    if (!empty($event_str)) {
      ...
      if ('0' == strpos(substr($event_str, 2), '%%')) { //if this code is executed, then file consisted of one line causing previous tactic to fail
      ...
      foreach ($event_key_pairs as $line) {
        ...
        if (in_array($date_key, $this->DTfields)) {
          ...
          foreach (explode(',', $value) as $date_value) {
            ...
            if (';VALUE=DATE' == $date_format) {
              $date_value .= 'T000000';
            }

            // date-time in UTC
            if ('Z' == substr($date_value, -1) || !in_array($timezone, DateTimeZone::listIdentifiers())) {
              $timezone = 'UTC';
            }
